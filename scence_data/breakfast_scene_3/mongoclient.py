...
class MongoWrapper():
    ...
    def get_object_image(self, obj_entry, ts):
        ...
        for annotation in annotations:
            if annotation['_type'] == 'rs.annotation.Detection':
                gt = annotation['name']
                break
    ...
    def save_object_hypotheses_for_scene(self, ts):
        ...
        if scene_doc.count() != 0:
            ...
            for hyp in hyps:
                ...
                cv2.imwrite(images["gt"] + "_" + str(idx) + '_rgb_' + str(ts) + '.png', images["rgb"])
                cv2.imwrite(images["gt"] + "_" + str(idx) + '_depth_' + str(ts) + '.png', images["depth"])
                cv2.imwrite(images["gt"] + "_" + str(idx) + '_mask_' + str(ts) + '.png', images["mask"])
                idx += 1
...
if __name__ =="__main__":
    ...
    mw=MongoWrapper("breakfast_scene_3")
